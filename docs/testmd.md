...
of FF-ICE using FIXM. It introduces the FF-ICE Application Library,
identifies a candidate set of FIXM-Based services enabling (part of) the
information exchanges defined by the ICAO ATMRPP in the Manual on FF-ICE
Implementation Guidance \[10\] and provides detailed examples of
FF-ICE/R1 Services realizations.

-   **Guidance for** **Translating FF-ICE FIXM Messages to ATS
    Messages**

This chapter defines a formal mapping between the FIXM Logical Model and
Air Traffic Services (ATS) message content as defined in ICAO Doc 4444
\[8\].
...
-   FIXM Core 4.2.0 supports nillable properties (which FIXM 4.1.0 did
    not)[1](https://www.fixm.aero/fixm_410.pl) that enable proper FF-ICE
    Flight Plan updates as described in the FF-ICE/R1 Implementation
    Guidance Manual;
...
can be posted at any time to the [FIXM Work
Area](https://ost.eurocontrol.int/sites/FIXM/SitePages/Home.aspx), using
the FIXM Community discussion forum. Improvement proposals can also be
sent by email to the FIXM CCB (<FIXM.CCB@eurocontrol.int>) or
alternatively to the FIXM Secretariat
(<fixm.secretariat@eurocontrol.int>).
...
1.  [FIXM 4.1.0 Core](https://www.fixm.aero/fixm_410.pl), including the
    -   [FIXM 4.1.0 Core XML
        Schemas](https://www.fixm.aero/releases/FIXM-4.1.0/FIXM_Core_v4_1_0_Schemas.zip)
        (Zip archive)
    -   [Schema
...
5.  [FIXM web site](http://www.FIXM.aero)
6.  [FIXM Work
...
1.  [ATMRPP/3-WP/766](https://ost.eurocontrol.int/sites/FIXM/Shared%20Documents/ICAO%20ATMRPP%20inputs%20for%20FIXM%204.1.0/ATMRPP3_WP_766_FF-ICE1%20Implementation%20Guidance_All.pdf)

-   “Manual on FF-ICE Implementation Guidance” 

1.  ICAO Doc 7910: Location Indicators
...
-   9854\](<http://www.icao.int/NACC/Documents/Meetings/2012/ASBU/Reference3.pdf>):
    > Global Air Traffic Management Operational Concept, 1st edition

1.  PAN AIDC ICD: PAN Regional (NAT and APAC) Interface Control Document

-   for ATC Interfacility Data Communications (PAN AIDC ICD), version
    > 1.0

1.  ICAO Doc 10039: Manual on System Wide Information Management (SWIM)

-   Concept

1.  ATMRPP-WG/24-WP/564: Flight Plan Filing Provisions for FF-ICE
...
?&gt; Example of FIXM core usage satisfying the requirement on data
structure:
...
!&gt; Example of FIXM core usage **NOT** satisfying the requirement on
data structure:
...
Figure 3: XSD Restriction vs XSD Profile
...
-   Copyright (c) US Federal Aviation Administration (FAA), available on
    . This extract features a class named “NasFlight” (in blue on the
    diagram) that extends the core model element “Extension” and which
    defines a content that supplements the core model element “Flight”.
    The content of class “NasFlight” does not replace or supersede any
    of the existing properties of the core “Flight” class. The example
    therefore qualifies as valid usage of the extension mechanism.
...
The co-ordinates are represented in FIXM by a two-valued
sequence[4](https://www.fixm.aero/releases/FIXM-4.1.0/FIXM_Core_v4_1_0_Modelling_Best_Practices.pdf),
...
within the FIXM (Base) namespace[5](http://www.FIXM.aero).
...
information in a digital format within a specific
domain*[6](https://ost.eurocontrol.int/sites/FIXM/SitePages/Home.aspx)
(e.g. AIXM for the aeronautical information domain or FIXM for the
flight information domain). However, some ATM operations may require ATM
...
The FF-ICE Application Library is an Application
Library[7](https://www.fixm.aero/fixm_nas_extension_420.pl) for FIXM
...
FF-ICE Implementation Guidance Manual, Appendix B
...
Table 1: Correspondences between FF-ICE Message templates and their ICAO
Doc 9965 Volume II description
...
beyond the scope of FIXM. This chapter only provides an example of
service description of FF-ICE/R1 Service implementations in order to
...
Figure 42: Arrival Aerodrome Object Model
...
Figure 43: Destination and Alternate Object Model
...
Figure 44: En-Route Alternate Object Model
...
> dd1mm1\[NS\]ddd2mm2\[EW\]

which is converted by

> decimal latitude = dd1 + (mm1/60)
>
> decimal longitude = ddd2 + (mm2/60)
...
&lt;fx:Flight
xmlns:fx="[http://www.fixm.aero/flight/4.2"](http://www.fixm.aero/flight/4.2")
xmlns:fb="[http://www.fixm.aero/base/4.2"](http://www.fixm.aero/base/4.2")&gt;
...
xmlns:msg="[http://www.fixm.aero/app/msg/1.0"](http://www.fixm.aero/app/msg/1.0")
xmlns:fx="[http://www.fixm.aero/flight/4.2"](http://www.fixm.aero/flight/4.2")
xmlns:fb="[http://www.fixm.aero/base/4.2"](http://www.fixm.aero/base/4.2")&gt;
...
(“[http://www.fixm.aero/app/example/1.0”](http://www.fixm.aero/app/example/1.0”))
and root element(“ExampleMessage”) as well as a number of header fields
needed to represent data XAS wants to exchange with each alert
(“sender”, “recipient”, “timestamp”, and “type”). XAS then goes on to
create two templates: one that locks down the content of a departure
alert and another for the arrival alert. Details on how to build this
Application Library along with more specifics as to its content are
supplied below in Appendix A.
...
xmlns:xmg="[http://www.fixm.aero/app/example/1.0"](http://www.fixm.aero/app/example/1.0")
xmlns:fb="[http://www.fixm.aero/base/4.2"](http://www.fixm.aero/base/4.2")
xmlns:fx="[http://www.fixm.aero/flight/4.2"](http://www.fixm.aero/flight/4.2")
...
(“[http://www.fixm.aero/ext/example/1.0”](http://www.fixm.aero/ext/example/1.0”))
as well as a number of fields XAS wishes to add to what is currently
available in FIXM (“sequenceNumber” and “position”). Based on lessons
learned during their work with departure and arrival alerts, XAS choses
to apply the Extension to Basic Message as well as Core to take
advantage of Basic Message’s existing MessageCollection element. This
allows XAS the option of using Message as a root element if they wish to
send single position reports or MessageCollection if they wish to send
many at once. Details on how to build this Extension along with more
specifics as to its content are supplied below in Appendix B.
...
&lt;msg:Message
xmlns:xmp="[http://www.fixm.aero/ext/example/1.0"](http://www.fixm.aero/ext/example/1.0")
xmlns:fx="[http://www.fixm.aero/flight/4.2"](http://www.fixm.aero/flight/4.2")
xmlns:fb="[http://www.fixm.aero/base/4.2"](http://www.fixm.aero/base/4.2")
xmlns:msg="[http://www.fixm.aero/app/msg/1.0"](http://www.fixm.aero/app/msg/1.0")
xmlns:xsi="[http://www.w3.org/2001/XMLSchema-instance"](http://www.w3.org/2001/XMLSchema-instance")&gt;
...
xmlns:msg="[http://www.fixm.aero/app/msg/1.0"](http://www.fixm.aero/app/msg/1.0")
xmlns:xmp="[http://www.fixm.aero/ext/example/1.0"](http://www.fixm.aero/ext/example/1.0")
xmlns:hdf="[http://www.fixm.aero/ext/handoff/1.0"](http://www.fixm.aero/ext/handoff/1.0")
elementFormDefault="qualified" version="1.0.0"&gt;

&lt;xs:import
namespace="[http://www.fixm.aero/base/4.2"](http://www.fixm.aero/base/4.2")
schemaLocation="../../core/base/Base.xsd"/&gt;

&lt;xs:import
namespace="[http://www.fixm.aero/flight/4.2"](http://www.fixm.aero/flight/4.2")
schemaLocation="../../core/flight/Flight.xsd"/&gt;

&lt;xs:import
namespace="[http://www.fixm.aero/app/msg/1.0"](http://www.fixm.aero/app/msg/1.0")
schemaLocation="../../applications/basicmessage/BasicMessage.xsd"/&gt;

&lt;xs:import
namespace="[http://www.fixm.aero/ext/example/1.0"](http://www.fixm.aero/ext/example/1.0")
schemaLocation="../example/Example.xsd"/&gt;

&lt;xs:import
namespace="[http://www.fixm.aero/ext/handoff/1.0"](http://www.fixm.aero/ext/handoff/1.0")
...
&lt;mesg:Message
xmlns:fb="[http://www.fixm.aero/base/4.1"](http://www.fixm.aero/base/4.1")
xmlns:fx="[http://www.fixm.aero/flight/4.1"](http://www.fixm.aero/flight/4.1")
xmlns:mesg="[http://www.fixm.aero/messaging/4.1"](http://www.fixm.aero/messaging/4.1")
...
1.  The operator, Delta Airlines, files the initial flight plan message
    (FPL) to KZOB, which is the departure center. This message is
    incorrect with regard to the flight’s route information (a “0” was
    placed in the unit of measure field for the airspeed in the Route
    Designator Field Type 15). A rejection message (rather than an ACK)
    is generated in response to this message because of the incorrect
    unit of measure. The rejection message is not shown. *Note: This is
    not something expected to happen in reality, but is an artificial
    case created to illustrate a logical reject of a message.*
...
1.  Delta Airlines re-files the corrected flight plan message to KZOB.
    The previously incorrect unit of measure field has been entered
    properly.
...
1.  Approximately 90 minutes prior to departure, Delta Airlines notifies
    Cleveland Center (KZOB) of a pre-departure delay (DLA) of some 40
    minutes due to awaiting passengers from a connecting flight.
...
1.  69 minutes prior to the expected departure time, the flight plan is
    transmitted to CZYZ Center.
...
1.  CZYZ immediately responds with a logical acknowledgment message
    (LAM) from the ATS system.
...
1.  When the flight departs, 17 minutes after the revised off block
    time, KZOB sends departure message to CZYZ.
...
1.  CZYZ immediately responds with a logical acknowledgement message
    from the ATS system.
...
1.  When a flight plan is established, an estimate message (EST) is sent
    by KZOB to CZYZ notifying it of the expected transition for the
    flight.
...
1.  CZYZ immediately responds with a logical acknowledgement message to
    the EST message.
...
1.  A change or modification message (CHG) is shown augmenting the time
    at the fix position for the coordination handling between KZOB and
    CZYZ Centers.
...
1.  The coordination message (CDN) between the KZOB and CZYZ ATS
...
1.  An acceptance message (ACP) is then transmitted by CZYZ Center to
    ZOB for the coordination handling by Toronto Center.
...
1.  Prior to the coordination exchange with the United Kingdom an
    amended current flight plan (CPL) is transmitted from CZQZ (Gander
    Operations Center) to EGGX (Shanwick Operations Center).
...
**NUGRA DCT -EGLL0715 EGKK -PBN/A1B1C1D1L1O1S1T1 NAV/RNVD1E2A1
REG/N172DN EET/CZQX0240 EGGX0438 EISN0606 EGTT0631 SEL/EQAC
CODE/A120F3)**

1.  EGGX immediately responds with a logical acknowledgement message to
...
1.  A coordination message (CDN) is transmitted for flight control at
    fix RAKIL between CZQX and EGGX Centers.
...
1.  An acceptance message (ACP) is sent from EGGX Center to the CZQZ ATS
    computer for the coordination timing.
...
1.  The flight arrives per the actual flying time of 07 hours and 15
    minutes and the arrival message (ARR) is sent from EGTT to KZOB.

**FF KZOBZZQA**  
**241127 EGTTZDZX**  
**(ARREGTT/KZOB245KZOB/EGTT094-DAL18/A5773-KDTW0411-EGLL1127)**

1.  KZOB immediately responds with a logical acknowledgement message to
    the arrival message back to EGTT.
...
2.  Click the arrow \[\] next to Applications in Project Browser to
    expand the Applications container.
...
1.  *Target Namespace* set to:
    “[http://www.fixm.aero/app/example/1.0”](http://www.fixm.aero/app/example/1.0”)\[20\].
...
> IMPORTANT NOTE: Sparx EA will not automatically create any of the
> directories for the path to the file specified in the *Schema File*
> field. **They must be created outside of Sparx EA before generating
> the schemas.** FIXM is natively set up to generate its schema files
> under a directory named “schemas” that is located in the same
> directory as the Sparx EA file. To save time, rather than creating all
> of the directories needed by hand, you can instead:
...
> To add these tags, click on the third icon from the left \[\] in the
> *Tags* tab and fill in the *Tag* and *Value* fields similar to what is
> shown below.
...
1.  Click on the class you wish to add your field to. You will see an
    upward arrow icon \[\] appear near the upper right hand corner of
    your class.
...
1.  Click on *Connector Source* and then click on the third icon from
    the left \[\] to add a new tagged value. The *tag* field should be
    set to “position” and the *value* field to the correct numeric value
    as determined above. In this example, the flight association,
...
1.  Click on your new restricted class in the diagram. You will see an
    upward arrow icon \[\] appear near the upper right hand corner of
    the class. Click on this arrow and drag it over the class you are
    restricting. Then release the mouse button and choose
    “Generalization” from the list of options that pops up.
...
1.  Once again, locate the class you wish to restrict in Project Browser
    ...
    &lt;&lt;XSDcomplexType&gt;&gt;Arrival). Click on the arrow \[\] to
    the left of the class to display its associated attributes.
...
2.  Note any connectors that are no longer desired. Delete these by
    right clicking on them and choosing “Delete Connector”\[31\]. In
    this example, the “arrival” connector between ExampleDA\_Flight and
...
Files](#X6da751ea0cf07df598b956f29ffb35c931bf943) below for more
details).
...
1.  For each namespace your templates both import and restrict (in this
    example, only
    “[http://www.fixm.aero/flight/4.2”](http://www.fixm.aero/flight/4.2”)),
    create one sub-package under Includes. Like Includes itself, these
    sub-packages will be “Package Only”. Choose a *Name* appropriate to
    both your templates and the schemas they will import. In this
    example, one sub-package will be created and it should be named
    “ExampleFlight”.
...
schemas using the
“[http://www.fixm.aero/flight/4.2”](http://www.fixm.aero/flight/4.2”)
namespace in three locations. Once each under the two message templates
(ArrivalAlert and DepartureAlert) and once for the “Includes” file used
to gather together all instances of the flight namespace used throughout
the templates in one place. Below are the elsif clauses that can be used
to add the content needed for these three include only files:
...
1.  Fill in values for your schema properties as appropriate for your
    ...
    1.  *Target Namespace* set to:
        “[http://www.fixm.aero/ext/example/1.0”](http://www.fixm.aero/ext/example/1.0”)\[33\].
...
listed in the [Create an Extension Root
Package](#Xabb10ecdb993c67e46ee03da82474f39ff9752a) section above. The
main difference is that Extension subcomponents contain diagrams and
Extension classes.
...
1.  Next, apply the XSDschema stereotype to your subcomponent as
    ...
    Stereotype](#Xaba45d1484d28e0e0b1e8bf0bf14b187b333fea) section
    above.
...
> NOTE: For this and other subcomponents, you should be able to skip the
> step of setting up additional namespaces for imported packages. Sparx
> EA should automatically generate these as needed.
>
...
3.  Click on the third icon from the left \[\] at the top of the tab to
    add a new tag.
...
Content](#Xfc05112c7f9bfce48196d1bdd241a19501776b0) can be used to add
as many additional subcomponents as needed to hold additional extension
hooks or Extension-specific packages.
...
2.  Select the component of the model you would like to build in Project
    Browser\[37\]. For this example, we will be building FIXM Core
    (model available at
...
.eap file itself. Sparx EA will not automatically create any of the
directories for the path to the files specified in the *Filename*
...
Figure 16: Preliminary Flight plan submission example with rejected (REJ
or ACK+NON CONCUR) preliminary flight plan
...
(CONCUR) preliminary flight plan
...
Figure 19: Preliminary Flight plan submission example with accepted (ACK
and CONCUR) preliminary flight plan
...
Figure 20: FF-ICE interaction with preliminary flight plan that requires
negotiation (NEGOTIATE)

**Example implementation**

Figure 21: Preliminary Flight plan submission example with preliminary
flight plan that requires negotiation (NEGOTIATE)
...
rejected for having failed data acceptability (REJ)
...
ACT+NON ACCEPTABLE) filed flight plan
...
1.  ***Planning Service***
    ...
    1.  Input messages:
        ...
        3.  ***Flight Cancellation Message***\[39\] as specified in
            \[10\] Page II-E-17 Section C-8
...
2.  ***Filing Service***
    ...
    1.  Input messages:
        ...
        3.  ***Flight Cancellation Message8**,* as specified in \[10\]
...
1.  **Generate JAXB-RI-Bound Source Code**
...
-   submitted flight plan. All business logic for your project should be
    > within the calling scope of the skeleton.
...
[1](https://www.fixm.aero/fixm_410.pl) To do so, FIXM has transitioned
from attributes to allowing for the model-wide support of “nillable”
properties.
...
[3](https://www.fixm.aero/documents/FIXM%20Strategy.pdf) When newer
versions of FIXM products are released, upgrading the restrictions only
requires updating the reference to the newer versions and implementing
the ad-hoc adaptations only for the parts that have changed.

[4](https://www.fixm.aero/releases/FIXM-4.1.0/FIXM_Core_v4_1_0_Modelling_Best_Practices.pdf)
FIXM does not use GML but mimics it for geographic positions. GML
encodes geographic locations as sequences of values since it employs the
same construct to represent polygons.

[5](http://www.FIXM.aero) This design is intentional. It saves FIXM from
being tied to an external standard for such a small use case and also
aims to avoid potential issues or difficulties when marshalling /
unmarshalling the standard xlink:href attribute.

[6](https://ost.eurocontrol.int/sites/FIXM/SitePages/Home.aspx) From
draft Volume I of the ICAO Manual on SWIM (ICAO Doc 10039)

[7](https://www.fixm.aero/fixm_nas_extension_420.pl) See chapter 2.2.2
for a general introduction to the concept of Application Library.
...
\[11\] It is assumed that validation of the flight plan ensures when
code ‘N’ is included in field 10a, no other code is included, but such
validation is not part of the translation rules.

\[12\] It is assumed that validation of the flight plan ensures the
field 10a code ‘R’ is always paired with field 18 PBN, but such
validation is not part of the translation rules.

\[13\] It is assumed that validation of the flight plan ensures the
...
\[14\] It is assumed that validation of the flight plan ensures when
code ‘N’ is included in field 10b, no other code is included, but such
validation is not part of the translation rules.

\[15\] If field 18 DOF is omitted it is necessary to apply business
...
\[16\] Note that each Extension must target the same version of FIXM
Core and/or Application Library in order to be used together. For
example, you cannot combine one Extension that uses FIXM Core 4.2.0 with
another Extension that uses FIXM Core 4.1.0.

\[17\] Starting with the Basic Message v1.0.0 model and then deleting
the BasicMessage package allows you to skip creating and setting up the
...
\[21\] This example used “xmg” but the prefix can be set to any value
that makes sense in the context of your Application. Because it will be
used throughout your generated schemas, a short prefix is typically
preferred.

\[22\] If your toolbox is not showing XML Schema options, make sure
you’ve applied the XSDschema stereotype to your package and close and
...
\[26\] Note, this only applies to the package itself – not the contents
of the package. Creating the contents of the message template is covered
in [Create Template Contents](#create-template-content).
...
model, including Core! It is very important that you do not do this.
...
\[34\] This example used “xmp” but the prefix can be set to any value
that makes sense in the context of your extension. Because it will be
used throughout your generated schemas, a short prefix is typically
preferred.
...
\[38\] This Perl script was developed using perl 5, version 16,
subversion 3 (v5.16.3) built for x86\_64-linux-thread-multi and
originally tested in a Linux environment (CentOS Linux 7 (Core)). It was
also tested using perl 5, version 28, subversion 1 (v5.28.1) built for
